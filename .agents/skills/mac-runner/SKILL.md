---
name: mac-runner
description: Operate the self-hosted GitLab Runner on the Mac that builds 2140.wtf's iOS IPA. Covers SSH access, restarting the runner, viewing logs, updating Xcode, debugging fastlane locally, and rotating match certificates.
---

# Mac Runner Operations

2140.wtf's iOS pipeline runs two CI jobs on a self-hosted GitLab Runner on a MacBook in the rack: `build-ipa` (signs and builds the IPA via Xcode + fastlane match) and `publish-app-store` (uploads the IPA via `fastlane deliver`, which shells out to Apple's iTMSTransporter — that tool only ships inside Xcode, so this job can't run on Linux). This skill covers operating the Mac.
...
This produces the IPA at `../artifacts/2140.wtf.ipa` exactly like CI. Add `--verbose` for detailed output.
...
```bash
export IPA_PATH="$HOME/Projects/ditto/artifacts/2140.wtf.ipa"
