 ...
 *
 * Connects the zero-dependency native passkey module with the app's Nostr login.
 * Generates Nostr keypairs, encrypts the private key with a passkey-derived master key,
 * and stores it locally. No server interaction required.
 */
...
const NATIVE_PASSKEY_ENC_SK_KEY = "2140_native_passkey_encrypted_sk";
const NATIVE_PASSKEY_PUBKEY_KEY = "2140_native_passkey_pubkey";
const NATIVE_PASSKEY_METHOD_KEY = "2140_native_passkey_method";
...
export async function registerNativePasskeyAccount(): Promise<NativePasskeyRegistrationResult> {
  ...
  try {
    ...
    if (errMsg.includes("PRF_NOT_SUPPORTED")) {
      try {
        await registerYubiKeyWithPrf(masterKey);
        method = "prf";
      } catch (yubiErr: unknown) {
        const yubiMsg = yubiErr instanceof Error ? yubiErr.message : "";
        if (yubiMsg.includes("PRF_NOT_SUPPORTED")) {
  ...
  localStorage.setItem(NATIVE_PASSKEY_ENC_SK_KEY, cipherText);
...
export async function loginNativePasskeyAccount(): Promise<NativePasskeyIdentity> {
  ...
  const secretKeyBytes = decoded.data as Uint8Array;
  const secretKey = bytesToHex(secretKeyBytes);
  const pubkey = getPublicKey(secretKeyBytes);
  const npub = nip19.npubEncode(pubkey);
...
export function removeNativePasskeyAccount(): void {
  removePasskeyEnrollment();
  try { localStorage.removeItem(NATIVE_PASSKEY_ENC_SK_KEY); } catch { /* ignore */ }
  try { localStorage.removeItem(NATIVE_PASSKEY_PUBKEY_KEY); } catch { /* ignore */ }
  try { localStorage.removeItem(NATIVE_PASSKEY_METHOD_KEY); } catch { /* ignore */ }
}
