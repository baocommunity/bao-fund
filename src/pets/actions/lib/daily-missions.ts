...
export const DAILY_MISSION_POOL: DailyMissionDefinition[] = [
  // ── Baby/Adult only ──────────────────────────────────────────────────────
  {
    ...
    description: 'Interact with your NOSTR PET 3 times',
...
export function totalDailySats(missions: MissionsContent): number {
  const base = missions.daily.reduce((sum, m) => {
    const def = POOL_BY_ID.get(m.id);
    return sum + (def && isMissionComplete(m) ? def.satsReward : 0);
  }, 0);
  const bonus = areAllDailyComplete(missions) ? DAILY_BONUS_SATS : 0;
  return base + bonus;
}

/** Sats earned by a specific daily mission (0 if incomplete or unknown) */
export function missionSats(missionId: string, mission: Mission): number {
  const def = POOL_BY_ID.get(missionId);
  if (!def || !isMissionComplete(mission)) return 0;
  return def.satsReward;
}
