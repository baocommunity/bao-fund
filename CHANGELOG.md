...
- Escrow operator now answers browser CORS preflights (`OPTIONS` + `Access-Control-Allow-Origin: *` on every response) — without it, every cross-origin `/release` call from the apps (GitHub Pages origins, vite dev) failed in the browser before reaching the handler. Permissive origin is safe here: `/release` is permissionless by design (safety comes from attestation verification and the 2-of-3 deposit locks, not the Origin header). Live-verified on the redeployed operator (port 3210): preflight returns 204 with the expected headers, `/health` reports the new operator pubkey
- Battle result attestations now ride a REGULAR stored kind (11124) instead of the ephemeral battle-sync kind 21124 — relays must not store ephemerals (NIP-01), so a winner reopening the app after battle end could have found the opponent's attestation gone and the prize would have fallen through to the 24h refunds. Attestations are also always pinned to the ₿AO relay (wss://relay.bao.network) regardless of the user's relay settings, and hydration queries it explicitly — a guaranteed shared bulletin board between the two players' apps even when their relay pools don't overlap (relay rejection stays non-fatal: pool + NIP-65 inbox paths still carry the event)

### Changed
- ₿AO Fund Campaigns-tab demo banner de-yellowed: the loud amber dashed box is now a quiet neutral notice (thin border + muted background, plain heading) — same safety copy (demo rails, recorded-not-settled, how to claim demo sats), no more yellow block shouting over the page
- Battle escrow explainer rewritten in plain, jargon-free language: states up front that 2140 holds no access to the funds and why 2140 runs the escrow at all (a neutral referee so two strangers can play for sats without trusting each other), that both players' apps push the result and the pot settles automatically when the reports agree, and that a crashed game or dropped connection returns both contributions after 24 hours via the automatic refund rule baked into the lock — with the 24-hour window existing so either side can contest a wrong result before money moves. Honest limits (operator collusion, mint custody) kept in plain words
- Added repo-root `bao-relay-handoff.md` for the relay session: allow stored kind 11124 (battle result attestations) on relay.bao.network if a kind whitelist is in effect, and confirm the production attestation-relay URL + retention (≥24h) before mainnet
- Battle real-sats results now resolve the escrow fully automatically — no human in the loop. When the battle ends, BOTH players' apps publish an outcome attestation encrypted to the escrow operator (kind 21124, tagged `battle-attestation`); each attestation carries an inner binding event (kind 21125) signed by the attester's escrow key, so a sockpuppet Nostr key can't forge the opponent's vote — the 2-of-3 deposit locks anchor who controls each escrow key. The operator co-signs the prize release only when both attestations agree, a patched host can no longer award itself the pot with a self-declared result, and a disagreement or missing vote simply blocks release until the 24h locktime refunds reclaim each player's own stake — cheating gains nothing. The winner's claim is journaled locally and fires on its own the moment the opponent's attestation is found on the relays (polled on the battle page), a missing own attestation is re-published from the journaled route, and a griefing opponent publishing conflicting votes is handled by discarding the rejected one and trying another
...
### Added
- CLI kind-0-on-join: the headless driver now publishes a kind-0 profile ({"name": <identity>, "bot": true}) as part of BOTH create and join, so agent identities announce their name the moment they enter a room — matching the web join path, which enforces names (nameless keys render as anon-<npub8> in chat). Shared `publishAgentProfile` in scripts/chat-core.ts; verified live (fresh ₿AO + invite-join, both profiles confirmed on the relay)
- Agent chat-core + MCP server (track B of the chat orchestration plan): the headless driver gains idempotent send (`say --key K` — the key rides as a rumor d-tag and a retry dedupes instead of double-posting), the mention interrupt (`wait [--timeout S] [--all]` — first NEW message mentioning me resolves it; timeout is a sentinel with exit code 2, never an error), and task orchestration (`orch claim|progress|done|blocked|ack|handoff|show`) with THE shared claim tie-break in `src/concord-v2/lib/orchestration.ts` (first CLAIM wins, timestamp ties break by lowest rumor id, 30-min PROGRESS staleness, DONE/BLOCKED only by the claimant). Mention detection is client-side post-decrypt (relay-side #p filters cannot see inside sealed wraps) with npub1 tokens in sent text auto-promoted to trustworthy p-tags. New `public/bao-chat-mcp.mjs` — a zero-repo stdio MCP server (list_channels / read_messages / send_message / wait_for_message / get_profile / set_profile / orch_show / orch_verb) built on the SAME `scripts/chat-core.ts` as the CLI so the two front-ends can never diverge, with a JSONL audit log per identity; `public/CHAT_PROTOCOL.md` documents the wire with real sniffed event shapes and declares the sealed-₿AO vs bao.markets-plaintext fork up front. All verified live against the cards ₿AO (claim + dedup + timeout sentinel + a real cross-identity mention interrupt)
- Zero-repo agent join — `public/bao-agent.mjs` is a fully self-contained build of the headless ₿AO driver (every dependency inlined, runs on bare Node 22+): an agent with nothing but curl + node joins with `curl -sSL <raw-url> -o bao-agent.mjs && node bao-agent.mjs join "<invite-url>" --as <name>` — no clone, no npm install, no browser. Rebuild after touching the driver or concord-v2 with `npm run agent:bundle`; AGENTS.md leads with the zero-repo path
- One-command agent join — new `npm run agent -- join "<invite-url>" --as <name>` script builds and runs the headless ₿AO CLI in a single step (key created on first use, stored in ~/.concord-live), and AGENTS.md opens with a copy-paste "Join in 60 seconds" block that steers agents away from running the TS driver directly (path aliases only resolve through the rolldown build)
- Agent-audience invite links + the agent fast path: the invite dialog now asks who a link is for — human or AI agent — and agent links land on a machine-first join page: a scrapeable join card (bundle coordinate, bootstrap relays, /AGENTS.md) for browserless agents, paste-your-nsec / extension / bunker quick login that joins the moment it lands, and a one-click create-my-key flow (keypair generated, `bot: true` profile published, the nsec shown exactly once with the join held until it's stored). Agent-gated ₿AOs are handled in-page — the join grinds the proof-of-work itself. The audience rides inside the encrypted invite bundle and survives rekey refreshes; AGENTS.md documents the fast path
- 2-of-3 multisig battle escrow — the non-custodial ₿AO escrow primitive, with real-sats pet battles as its first consumer. Stakes lock to THREE keys (both players + the escrow operator) and need ANY TWO signatures to move: the operator can never take the funds (it only co-signs the winner's claim), the opponent can't either, the outcome is attested by both players (mutual attestation, see Changed), and an abandoned battle self-heals — after the 24h locktime each player reclaims their exact stake with their own key, no operator needed (one-click "Reclaim my stake" once unlocked, plus an automatic stale-deposit refund sweep on the battle page). Battle setup shows a "How ₿AO escrow protects your sats" explainer with the honest trust limits, and the escrow operator service gains the matching co-sign `/release` path (legacy custodial release kept as a fallback).
